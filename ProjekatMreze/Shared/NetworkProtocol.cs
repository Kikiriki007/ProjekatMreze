...
namespace invaders.Shared
{
    ...
    {
        private static readonly byte[] PACKET_HEADER = { 0x49, 0x4E, 0x56 };// inv heksadec za header

        public static byte[] Serialize<T>(T obj)  //prebacivanje u bitove
        {
            ...
            byte typeId = GetTypeId<T>();//uzimmamo tip objekta, pisemo taj bajt tog tipa
            ms.WriteByte(typeId);
        ...
        public static byte[] PackWithLength(byte[] data)
        {
            ...
            byte[] result = new byte[4 + data.Length];//rezervise 4 bajta da bi ogranicio pocetak i kraj
        ...
        public static byte[] ReadWithLength(Stream stream)
        {
            ...
            if (bytesRead < 4) return null;//corrupted data

            int length = BitConverter.ToInt32(lengthBytes, 0);
            if (length <= 0 || length > 1024 * 1024)//da li je veci od mega bajt 
                return null;
